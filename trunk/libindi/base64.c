 ...
 */
int
from64tobits(char *out, const char *in, int *length)
{
    int len = 0;
	...
	while (isspace(*in))
	    in++;
    } while (*in && digit4 != '=' && (!length || len < *length));
    if (length)
        *length = in - orig_in;
    return (len);
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: base64.c,v $ $Date: 2009/07/31 02:20:34 $ $Revision: 1.1 $ $Name:  $"};
